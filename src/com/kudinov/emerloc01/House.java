...
public class House {
    ...
    public void add(Node node, int i, int j, int width, int height) {
        ...
        node.setHouse(this);
        if (node.verify(NodeType.HOUSE_BLOCK, LEFT) && j != 0) {
            add(node.getLeftNode(), i, j - 1, width, height);
        }
        if (node.verify(NodeType.HOUSE_BLOCK, RIGHT) && j != width - 1) {
            add(node.getRightNode(), i, j + 1, width, height);
        }
        if (node.verify(NodeType.HOUSE_BLOCK, UP) && i != 0) {
            add(node.getUpNode(), i - 1, j, width, height);
        }
        if (node.verify(NodeType.HOUSE_BLOCK, DOWN) && i != height - 1) {
