...
public class WaterSupplyMap {
        ...
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[i].length; j++) {
                ...
                if (node.isPipelineBlock()) {
                    ...
                    if (!node.verify(NodeType.HOUSE, UP) &&     //cross
                            !node.verify(NodeType.HOUSE, LEFT) &&
                            !node.verify(NodeType.HOUSE, RIGHT) &&
                            !node.verify(NodeType.HOUSE, DOWN)) {
                        pipe.addParts(LEFT, UP, RIGHT, DOWN);
                        continue;
                    }

                    if (!node.verify(NodeType.HOUSE, LEFT) &&  //up valve
                            !node.verify(NodeType.HOUSE, UP) &&
                            !node.verify(NodeType.HOUSE, RIGHT) &&
                            (node.verify(NodeType.HOUSE, DOWN))) {
                    ...
                    if ((!node.verify(NodeType.HOUSE, RIGHT) && !node.verify(NodeType.HOUSE, LEFT)) && //horizontal
                            ((node.verify(NodeType.HOUSE, UP) && (node.verify(NodeType.HOUSE, DOWN) ||
                                    (!node.verify(NodeType.HOUSE, RIGHT, DOWN) || !node.verify(NodeType.HOUSE, LEFT, DOWN)) ||
                                    (node.verify(NodeType.HOUSE, DOWN) && (!node.verify(NodeType.HOUSE, RIGHT, UP) ||
                                            !node.verify(NodeType.HOUSE, LEFT, UP))))))) {
                        pipe.addParts(LEFT, RIGHT);
                        continue;
                    }


                    if (!node.verify(NodeType.HOUSE, DOWN) && !node.verify(NodeType.HOUSE, RIGHT) && // down valve
                            !node.verify(NodeType.HOUSE, LEFT) &&
                            (node.verify(NodeType.HOUSE, UP))) {
                    ...
                    if (!node.verify(NodeType.HOUSE, LEFT) && !node.verify(NodeType.HOUSE, DOWN) && //left down corner
                            node.verify(NodeType.HOUSE, LEFT, DOWN) && node.verify(NodeType.HOUSE, RIGHT) &&
                            (node.verify(NodeType.HOUSE, UP) || !node.verify(NodeType.HOUSE, LEFT, UP))) {
                        pipe.addParts(LEFT, DOWN);
                        continue;
                    }


                    if (!node.verify(NodeType.HOUSE, LEFT) && node.verify(NodeType.HOUSE, LEFT, UP) &&  //left up corner
                            !node.verify(NodeType.HOUSE, UP) && node.verify(NodeType.HOUSE, RIGHT) && node.verify(NodeType.HOUSE, DOWN)) {
                        pipe.addParts(LEFT, UP);
                        continue;
                    }


                    if (node.verify(NodeType.HOUSE, LEFT) && node.verify(NodeType.HOUSE, DOWN) && // right up corner
                            node.verify(NodeType.HOUSE, RIGHT, UP) &&
                            !node.verify(NodeType.HOUSE, UP) && !node.verify(NodeType.HOUSE, RIGHT)) {
                        pipe.addParts(RIGHT, UP);
                        continue;
                    }

                    if (node.verify(NodeType.HOUSE, LEFT) && !node.verify(NodeType.HOUSE, DOWN) && //right down corner
                            !node.verify(NodeType.HOUSE, RIGHT) && node.verify(NodeType.HOUSE, RIGHT, DOWN) &&
                            (node.verify(NodeType.HOUSE, UP) || !node.verify(NodeType.HOUSE, RIGHT, UP))) {
                        pipe.addParts(RIGHT, DOWN);
                        continue;
                    }

                    if (node.verify(NodeType.HOUSE, RIGHT) && !node.verify(NodeType.HOUSE, UP) && // left valve
                            !node.verify(NodeType.HOUSE, DOWN) && !node.verify(NodeType.HOUSE, LEFT) &&
                            (node.verify(NodeType.HOUSE, LEFT, UP) || node.verify(NodeType.HOUSE, LEFT, DOWN))) {
                    ...
                    if (node.verify(NodeType.HOUSE, LEFT) && !node.verify(NodeType.HOUSE, RIGHT) && //right valve
                            !node.verify(NodeType.HOUSE, UP) && !node.verify(NodeType.HOUSE, DOWN) &&
                            (node.verify(NodeType.HOUSE, RIGHT, UP) || node.verify(NodeType.HOUSE, RIGHT, DOWN))) {
    ...
    public void addWaterIntake() {
        ...
        for (House house : sortedHouses) {
            ...
            for (Node node : houseFragments) {
                if (node.getJ() != 0 && node.verify(NodeType.PIPELINE_BLOCK, LEFT) &&
                    ...
                    if (allHousesInGroupNotFull && node.getLeftNode().getJ() != 0 &&
                            node.verify(NodeType.HOUSE, LEFT, LEFT) &&
                ...
                if (node.getJ() != (structure.getWidth() - 1) &&
                        node.verify(NodeType.PIPELINE_BLOCK, RIGHT) &&
                    ...
                    if (allHousesInGroupNotFull &&
                            ...
                            node.verify(NodeType.HOUSE, RIGHT, RIGHT) &&
                ...
                if (node.getI() != 0 && node.verify(NodeType.PIPELINE_BLOCK, UP) &&
                    ...
                    if (allHousesInGroupNotFull && node.getUpNode().getI() != 0 &&
                            node.verify(NodeType.HOUSE, UP, UP) &&
                ...
                if (node.getI() != (structure.getHeight() - 1) &&
                        node.verify(NodeType.PIPELINE_BLOCK, DOWN) &&
                    ...
                    if (allHousesInGroupNotFull &&
                            ...
                            node.verify(NodeType.HOUSE, DOWN, DOWN) &&
    ...
    public void pipelineUnion() {
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[i].length; j++) {
                Node node = map[i][j];
                if (node.getType() == NodeType.PIPELINE_BLOCK && node.getPipe().containValve() &&
